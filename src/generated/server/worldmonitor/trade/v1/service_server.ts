...
export interface GetTradeDashboardRequest {
}

export interface GetTradeDashboardResponse {
  restrictions?: GetTradeRestrictionsResponse;
  tariffs?: GetTariffTrendsResponse;
  flows?: GetTradeFlowsResponse;
  barriers?: GetTradeBarriersResponse;
}
...
export interface TradeServiceHandler {
  ...
  getTradeDashboard(ctx: ServerContext, req: GetTradeDashboardRequest): Promise<GetTradeDashboardResponse>;
}
...
export function createTradeServiceRoutes(
  ...
  return [
    ...
    {
      method: "GET",
      path: "/api/trade/v1/get-trade-dashboard",
      handler: async (req: Request): Promise<Response> => {
        try {
          const pathParams: Record<string, string> = {};
          const body = {} as GetTradeDashboardRequest;
          ...
          const result = await handler.getTradeDashboard(ctx, body);
          return new Response(JSON.stringify(result as GetTradeDashboardResponse), {
            status: 200,
            headers: { "Content-Type": "application/json" },
          });
        } catch (err: unknown) {
