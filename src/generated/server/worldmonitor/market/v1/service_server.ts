...
export interface GetMarketDashboardRequest {
}
...
export interface MarketServiceHandler {
  ...
  getMarketDashboard(ctx: ServerContext, req: GetMarketDashboardRequest): Promise<GetMarketDashboardResponse>;
}
...
export function createMarketServiceRoutes(
  ...
  return [
    ...
    {
      method: "GET",
      path: "/api/market/v1/get-market-dashboard",
      handler: async (req: Request): Promise<Response> => {
        try {
          const pathParams: Record<string, string> = {};
          const body = {} as GetMarketDashboardRequest;
          ...
          const result = await handler.getMarketDashboard(ctx, body);
          return new Response(JSON.stringify(result as GetMarketDashboardResponse), {
            status: 200,
            headers: { "Content-Type": "application/json" },
          });
        } catch (err: unknown) {
