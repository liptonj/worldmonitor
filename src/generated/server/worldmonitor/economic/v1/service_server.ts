...
export interface GetBisDashboardRequest {
}

export interface GetBisDashboardResponse {
  policyRates: BisPolicyRate[];
  exchangeRates: BisExchangeRate[];
  creditGdp: BisCreditToGdp[];
}
...
export interface EconomicServiceHandler {
  ...
  getBisDashboard(ctx: ServerContext, req: GetBisDashboardRequest): Promise<GetBisDashboardResponse>;
}
...
export function createEconomicServiceRoutes(
  ...
  return [
    ...
    {
      method: "GET",
      path: "/api/economic/v1/get-bis-dashboard",
      handler: async (req: Request): Promise<Response> => {
        try {
          const pathParams: Record<string, string> = {};
          const body = {} as GetBisDashboardRequest;
          ...
          const result = await handler.getBisDashboard(ctx, body);
          return new Response(JSON.stringify(result as GetBisDashboardResponse), {
            status: 200,
            headers: { "Content-Type": "application/json" },
          });
        } catch (err: unknown) {
