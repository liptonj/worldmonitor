...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === 'POST') {
    ...
    const { error } = await client.schema('wm_admin').from('news_sources').insert(body);
    if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500, headers });
  ...
  if (req.method === 'PUT') {
    ...
    const body = await req.json();
    const { error } = await client.schema('wm_admin').from('news_sources').update(body).eq('id', id);
    if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500, headers });
  ...
  if (req.method === 'DELETE') {
    if (!id) return new Response(JSON.stringify({ error: 'id required' }), { status: 400, headers });
    const { error } = await client.schema('wm_admin').from('news_sources').delete().eq('id', id);
    if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500, headers });
    return new Response(JSON.stringify({ ok: true }), { status: 200, headers });
  }
